...
namespace EES.Infrastructure.Middleware
{
    ...
    public class ValidationMiddleWare
    {
        ...
        public static string ConvertJsonFormat(string inputJson)
        {
            try
            {
                ...
                // 返回解析到的错误消息，如果没有找到则返回"错误无法捕获"
                return errorMessage ?? "错误无法捕获";
            }
            catch (JsonReaderException)
            {
                // 如果解析JSON时发生异常，则返回"错误无法捕获"
                return "错误无法捕获";
            }
        }
    }
    ...
    public class ContextResponse
    {
        ...
        public static async Task ImmediateReturn(HttpContext context, int code, string message, string type = "application/json")
        {
            ...
            await context.Response.WriteAsync(ApiResponseBase.Fail(code, message).ToJson());
        }
    }
}